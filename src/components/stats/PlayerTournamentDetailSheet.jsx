...
export default function PlayerTournamentDetailSheet({
  ...
  const data = useMemo(
    () => playerId ? computePlayerTournamentBreakdown(playerId, allMatches, scoringLevel) : null,
    [playerId, allMatches, scoringLevel],
  )
  ...
  const hasDetailedBreakdown = scoringLevel >= 3

  // Filter point types relevant to scoring breakdown
  const scoringTypes = POINT_TYPES.filter(pt => pt.id !== 'error')
  const maxScored = hasDetailedBreakdown
    ? Math.max(1, ...scoringTypes.map(pt => data.byType[pt.id] || 0))
    : 1
  const maxError = hasDetailedBreakdown
    ? Math.max(1, ...ERROR_SUBTYPES.map(es => data.errorsByType[es.id] || 0))
    : 1

  const net = data.points - data.errors
  const netCls = net > 0 ? 'text-success' : net < 0 ? 'text-error' : 'text-dim'
