...
import {
  Flame, TrendingUp, Percent, Sigma, Trophy, Zap,
...
/**
 * Title trophies won by a player: one trophy per title up to 3, then a single
 * trophy with a ×N count so long win records stay compact on a 320px screen.
 */
function TitleTrophies({ count }) {
  if (!count) return null
  return (
    <span className="flex items-center gap-0.5 text-accent flex-shrink-0" title={`${count} title${count === 1 ? '' : 's'}`}>
      {count <= 3 ? (
        Array.from({ length: count }, (_, i) => <Trophy key={i} size={11} />)
      ) : (
        <>
          <Trophy size={11} />
 ...
 * trend, stat highlight strip, full leaderboard, league leaders and records,
 * and the tournaments list. Tapping a player opens their stats sheet.
 */
export default function RankingsTab({ league, isAdmin, isGuest, currentUserId }) {
  ...
  const { champions, highlights } = insights

  // playerId → tournaments won, for the trophy badges on the leaderboard
  const titlesByPlayer = useMemo(
    () => new Map((champions.titles || []).map(t => [t.playerId, t.titles])),
    [champions]
  )
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0 ml-2.5">
                      <div className="flex items-center gap-1.5">
                        ...
                        <TitleTrophies count={titlesByPlayer.get(player.id)} />
                        {streak?.current >= 3 && (
